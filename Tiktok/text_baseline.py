...
df_cap = df[df["Captions"].notna()].copy()
df_cap["text"] = df_cap["Captions"].apply(clean_text)
df_cap = df_cap[df_cap["text"].apply(valid_text)].reset_index(drop=True)
run_baseline(df_cap, "Captions only")

df_desc = df[df["Description"].notna()].copy()
df_desc["text"] = df_desc["Description"].apply(clean_text)
df_desc = df_desc[df_desc["text"].apply(valid_text)].reset_index(drop=True)
run_baseline(df_desc, "Descriptions only")

df_all = df[df["Captions"].notna() | df["Description"].notna()].copy()
df_all["text"] = (
    df_all["Captions"].apply(clean_text) + " " + df_all["Description"].apply(clean_text)
).str.strip()
df_all = df_all[df_all["text"].apply(valid_text)].reset_index(drop=True)
