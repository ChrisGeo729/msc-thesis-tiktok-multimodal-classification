...
OUT_DIR     = "/home/cgeorghiou/projects/msc-thesis/YouTube/analysis"
THR         = 0.3
TOP_N       = 20
...
models = {
    "Text (titles + TF-IDF tags)": {
        "prob":    np.load(f"{SCRATCH_DIR}/preds_text_prob.npy"),
    ...
    "Multimodal (early fusion)": {
        "prob":    np.load(f"{SCRATCH_DIR}/preds_multimodal_early_prob.npy"),
        "true":    np.load(f"{SCRATCH_DIR}/preds_multimodal_early_true.npy"),
        "classes": np.load(f"{SCRATCH_DIR}/preds_multimodal_early_classes.npy", allow_pickle=True),
    },
}
...
for ax, (name, m) in zip(axes, models.items()):
    df = m["metrics"]
    ax.scatter(df["support"], df["f1"], alpha=0.4, s=10,
...
fig.suptitle("YT8M — Label frequency vs F1 score per class (thr=0.3)", fontsize=14, y=1.02)
plt.tight_layout()
...
width = 0.25
for i, mname in enumerate(list(models.keys())):
...
ax.set_title("YT8M — Mean F1 by label frequency bin (thr=0.3)", fontsize=12)
ax.set_ylim(0, 1.05)
ax.legend(fontsize=9)
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
...
for ax, (name, m) in zip(axes, models.items()):
    im = co_occurrence_heatmap(m["prob"], m["true"], m["classes"], THR, TOP_N, name, ax)
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="Prediction rate")

fig.suptitle(f"YT8M — Co-occurrence heatmap — top {TOP_N} classes by support (thr={THR})",
             fontsize=13, y=1.02)
plt.tight_layout()
...
fig.suptitle("YT8M — Predicted probability distribution: positive vs negative labels",
             fontsize=13, y=1.02)
plt.tight_layout()
