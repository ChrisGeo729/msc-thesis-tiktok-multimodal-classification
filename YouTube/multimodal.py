...
TEXT_FEAT    = "/scratch-shared/cgeorghiou/textcnn_yt8m_titles_features.npy"
TEXT_IDS     = "/scratch-shared/cgeorghiou/textcnn_yt8m_titles_videoids.npy"
VISUAL_FEAT  = "/scratch-shared/cgeorghiou/visual_deep_yt8m_features.npy"
VISUAL_IDS   = "/scratch-shared/cgeorghiou/visual_deep_yt8m_videoids.npy"
SAVE_DIR     = "/scratch-shared/cgeorghiou"
...
df = df[df["YouTube_id"].isin(common_ids)].drop_duplicates(
    subset="YouTube_id", keep="first").reset_index(drop=True)
print(f"Videos with labels in intersection: {len(df):,}")

X_text   = np.array([text_lookup[vid]   for vid in df["YouTube_id"]], dtype=np.float32)
X_visual = np.array([visual_lookup[vid] for vid in df["YouTube_id"]], dtype=np.float32)
...
Y_prob_moe, Y_true_moe = evaluate(moe_model, test_loader)
print_metrics(Y_prob_moe, Y_true_moe, "MoE fusion (text + visual, 4 experts)")
...
np.save(f"{SAVE_DIR}/preds_multimodal_moe_prob.npy",      Y_prob_moe)
np.save(f"{SAVE_DIR}/preds_multimodal_moe_true.npy",      Y_true_moe)
